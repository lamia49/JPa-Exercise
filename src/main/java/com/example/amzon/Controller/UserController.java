...
public class UserController {
   private final UserService userService;
    ...
    @PutMapping("/buy/{userId}/{ProudctId}/{stockId}/{idMarchents}")

    public ResponseEntity buy(@PathVariable  int userId ,@PathVariable int ProudctId,@PathVariable int stockId ,@PathVariable int idMarchents ){
        boolean pay=userService.buy(userId,ProudctId,stockId,idMarchents);
        if(pay){
    ...
    @PutMapping("/refund/{userId}/{proudectID}")

    public ResponseEntity refund(@PathVariable int userId,@PathVariable int proudectID){

        boolean refund=userService.refund(userId,proudectID);
        if(refund){
