...
public class ProudctController {
   private final ProductService productService;
    ...
    @PutMapping("/update/{id}/{catigrayId}")
    public ResponseEntity update(@PathVariable int id,@PathVariable int catigrayId,@RequestBody @Valid Proudct proudct ,Errors error){
        if(error.hasErrors()){
            String massege= error.getFieldError().getDefaultMessage();
            return ResponseEntity.status(400).body(massege);
        }
        boolean isFound= productService.update(id,catigrayId,proudct);
        if(isFound){
